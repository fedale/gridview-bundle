...
class GridviewRealtimeConfigTest extends TestCase
{
    public function testRealtimeDisabledByDefault(): void
    {
        ...
        $realtime = $registry->resolveOptions(null)['behavior']['realtime'];
    ...
    {
        $registry = new GridviewConfigRegistry([
            'defaults' => ['behavior' => ['realtime' => ['enabled' => true, 'topicPrefix' => 'gridview/']]],
        ]);

        $this->assertTrue($registry->resolveOptions(null)['behavior']['realtime']['enabled']);
    }
    ...
    public function testRealtimeEnabledPerGridOnly(): void
    {
        $registry = new GridviewConfigRegistry([
            'gridviews' => [
                'customer' => ['behavior' => ['realtime' => ['enabled' => true, 'topicPrefix' => 'gridview/']]],
            ],
        ]);

        // The opted-in grid is enabled...
        $this->assertTrue($registry->resolveOptions('customer')['behavior']['realtime']['enabled']);
        // ...while others fall back to the disabled default.
        $this->assertFalse($registry->resolveOptions('user')['behavior']['realtime']['enabled']);
    }
}
