...
class FedaleGridviewBundle extends AbstractBundle
{
    ...
    public function configure(DefinitionConfigurator $definition): void
    {
        $definition->rootNode()
            ...
            // semantic significance to the split beyond that.
            ->arrayNode('display')
            ->addDefaultsIfNotSet()
            ->children()
            ...
            ->addDefaultsIfNotSet()
            ->children()
            ->scalarNode('default')->defaultValue('table')->end()
            ->arrayNode('map')
            ->useAttributeAsKey('name')
            ...
            // Full-page CRUD wrapper template; ex `crud.pageTemplate`, now
            // bridged from `viewConfig()['template']['page']` — see
            // AbstractCrudGridController::crudOptions().
            ->scalarNode('crudTemplate')->defaultNull()->end()
            ->arrayNode('layout')
            ->addDefaultsIfNotSet()
            ->children()
            ...
            ->defaultValue([])
            ->end()
            ->scalarNode('formName')->defaultValue('fedaleForm')->end()
            ->integerNode('maxQueryLength')->defaultValue(4000)->end()
            // 'modal' | 'page' | 'custom'; ex `form.mode`, now bridged from
            ...
            // Filter-bar auto-population; null = derived from the active
            // renderer (see Gridview::$options).
            ->variableNode('autoBar')->defaultNull()->end()
            ->integerNode('choiceControlsThreshold')->defaultValue(20)->end()
            ->end()
            ->end()
            ...
            ->end()
            ->end()
            ->booleanNode('reorderColumns')->defaultFalse()->end()
            ->booleanNode('responsive')->defaultFalse()->end()
            // Shows the {restrictionNotice} banner when the list is filtered
            // by permissions; a string overrides the default message.
            ->variableNode('restriction')->defaultFalse()->end()
            ->end()
            ->end()
            ...
            // Per-grid override only accepts the scalar shorthand (active
            // strategy name) — the full {default, map} shape lives in
            // `defaults.display.renderer` only.
            ...
            ->scalarPrototype()->end()
            ->end()
            ->scalarNode('formName')->end()
            ->integerNode('maxQueryLength')->end()
            ->scalarNode('crudMode')->end()
            ->arrayNode('filterControls')
            ->children()
            ->booleanNode('inHeader')->end()
            ->booleanNode('inlineClear')->end()
            ->variableNode('clear')->end()
            ->variableNode('autoBar')->end()
            ->integerNode('choiceControlsThreshold')->end()
            ->end()
            ->end()
            ->arrayNode('pagination')
            ->children()
            ...
            ->end()
            ->end()
            ->booleanNode('reorderColumns')->end()
            ->booleanNode('responsive')->end()
            ->variableNode('restriction')->end()
            ->end()
            ->end()
            ->arrayNode('integration')
            ->children()
