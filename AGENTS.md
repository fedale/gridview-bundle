...
- Do not construct compound bash commands using if/then, loops, or variables. Run tools one at a time with direct calls (e.g. php-cs-fixer fix --dry-run ...), letting the permission system evaluate them individually.
...
When changing behaviour, update the matching guide under `docs/`.
